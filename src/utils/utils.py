...
def get_fk_solution(joint_angles):
    """Franka Panda Forward Kinematics using DH parameters."""
    def get_tf_mat(i, dh):
        a = dh[i][0]
        d = dh[i][1]
        alpha = dh[i][2]
...
class ActionAdapter(nn.Module):
    def __init__(self, joint_dim=7, action_dim=7, hidden_dim=512, horizon=8):
        super().__init__()
        self.joint_dim = joint_dim
        self.action_dim = action_dim
        self.horizon = horizon
        
        # Maps (current_joints [7], future_velocities [H*7]) -> future_deltas [H*7]
        input_dim = joint_dim + horizon * action_dim
        output_dim = horizon * joint_dim
    ...
    def forward(self, current_joints, predicted_velocities):
        """
        current_joints: (B, 7)
        predicted_velocities: (B, H, 7)
        Returns: Cartesian poses (B, H, 7) - [x, y, z, qx, qy, qz, qw]
        """
        B = current_joints.shape[0]
        flat_vel = predicted_velocities.reshape(B, -1)
        x = torch.cat([current_joints, flat_vel], dim=1)
        
        # Predict joint deltas
        deltas = self.net(x).reshape(B, self.horizon, self.joint_dim)
        future_joints = current_joints.unsqueeze(1) + deltas
        
        # Convert to Cartesian via FK (CPU-bound)
        batch_poses = []
        future_joints_np = future_joints.detach().cpu().numpy()
        
        for b in range(B):
            poses = []
            for h in range(self.horizon):
                T = get_fk_solution(future_joints_np[b, h])
                pos = T[:3, 3]
                rot = R.from_matrix(T[:3, :3]).as_quat() # [qx, qy, qz, qw]
                poses.append(np.concatenate([pos, rot]))
            batch_poses.append(poses)
            
        return torch.tensor(np.array(batch_poses), device=current_joints.device, dtype=current_joints.dtype)
...
def rescale_droid_action(a, state_p01, state_p99):
    """Normalize DROID 7D state using min/max stats."""
    eps = 1e-8
    ndata = 2 * (a - state_p01) / (state_p99 - state_p01 + eps) - 1
    return torch.clamp(ndata, -1, 1)
